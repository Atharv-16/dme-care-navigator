# Clinic — Gemini Live `system_instruction`

Path: `ClinicAgent.handle` → `build_system_prompt(..., mode="clinic", spoken=True)` → `LiveVoiceServer._live_config`. Fabricated snapshot: first clinic ring, empty memory, order still `verbal_only`.
...
INPUT_CONTEXT_JSON:
{
  ...
  "current_call": {
    "target_id": "clinic",
    "target_name": "Sunrise Family Medicine",
    "call_type": "clinic",
    "goal": "Confirm whether the signed K0001 written order is ready.",
    "facts_to_share": [
      "Patient: Eleanor Martinez",
      "Date of birth: 1954-03-12",
      "Requested equipment: K0001",
      "Known order status: verbal_only"
    ],
    "questions": [
