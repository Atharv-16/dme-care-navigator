# Analyzer — Gemini chat JSON **system** message

Path: `analyze_call` → `_analyze_system_prompt()` → `build_system_prompt("navigator", mode="analyze")`, then `chat_json` appends the JSON-only line. **No** Live wrapper. **No** `INPUT_CONTEXT_JSON` here: context + transcript are the **user** message, not this system string. Snapshot not required.
...
Ground truth: {"patient": "Eleanor Martinez", "patient_dob": "1954-03-12", "patient_phone": "(312) 555-0190", "clinic": "Sunrise Family Medicine", "doctor": "Dr. Sarah Chen", "clinic_phone": "(312) 555-0198", "hcpcs": "K0001", "is_patient": false}

Return ONLY valid JSON with keys reply and conclusion: {"reply":"spoken words for TTS this turn","conclusion":""}. No markdown fences. Put [END] or [NO_ANSWER] inside reply when hanging up. On a live phone turn, conclusion is usually empty. After a call (analyze), put the self-contained case log in conclusion.
